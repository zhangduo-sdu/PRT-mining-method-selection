...
|---|---|
| Fourteen-dimensional input vector `[X1, ..., X11, sigma1, delta, Ap]` | `configs/prt_config.yaml`, `data/synthetic_benchmark.csv`, `src/preprocessing.py` |
...
| Predicted-vs-ground-truth parity plot | `src/metrics.py`, output `parity_plot.png` |
...
The code is executable without confidential mine data. The included
`data/synthetic_benchmark.csv` follows the manuscript feature schema and
workflow logic. The same scripts can be run on real data if the real feature
matrix is provided in the same column format as
`data/example_real_case_format.csv`.
